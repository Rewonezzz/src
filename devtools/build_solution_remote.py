# WHY WHY WHY
# This is high level stuff, do not touch this if you don't know what you're doing
# A lot of cleaning up and issues fixed.
#!/usr/bin/env python
...
# Python 2/3 compatibility for raw_input/input
try:
    input = raw_input
except NameError:
    pass
...
def usage():
    ...
    print("")
    print("-p,--platform=   	%s" % list(g_mapPlatformToBuildHostInfo.keys()))
...
def runLocalCommand( szCommand, bWait=True ):
    ...
    po = subprocess.Popen( szCommand, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=sys.stdin, universal_newlines=True )
    if bWait:
        (stdout_data, stderr_data) = po.communicate()
        if g_bDebugVerbose:
            print(stdout_data)
            print(stderr_data)
        return ( po.returncode, stdout_data, stderr_data )
    else:
        return ( 0, "", "" )
...
def runRemoteCommand( szRemoteUser, szRemoteHost, szCommand, bPromptForPasswd = False, bCheckReturn = True ):
    ...
    if sys.platform == 'win32':
        ...
        if bPromptForPasswd:
            ...
            # NOTE: plink's -pw exposes password in process command line (security limitation)
    ...
    if sys.platform == 'win32':
        ...
        # Write command as bytes (UTF-8 encoded) for Python 3 compatibility
        os.write( fd, szCommand.encode('utf-8') )
        os.close( fd )
        cmd += " -m %s" % fname
        if g_bDebugVerbose:
            print("tmpfile %s contains %s" % ( fname, szCommand ))
    else:
        cmd += " '%s'" % szCommand
    ...
    po = None
    try:
        po = subprocess.Popen( cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=sys.stdin, universal_newlines=True )
        (stdout_data, stderr_data) = po.communicate()
    finally:
        # Ensure temp file is deleted even if an exception occurs
        if fd is not None and fname is not None:
            try:
                os.unlink( fname )
            except OSError:
                pass  # best effort

    if po is not None and po.returncode != 0 and bCheckReturn:
        print("looks like something went wrong with a remote command")
        print(">>> %s " % cmd)
        print("returned %d" % po.returncode)
        if sys.platform == 'win32':
            print("tempfile contained %s" % szCommand)
    ...
    if g_bDebugVerbose:
        print("return: %d\nstdout: %s\nstderr: %s" % ( po.returncode if po else -1, stdout_data, stderr_data ))

    return ( po.returncode if po else -1, stdout_data, stderr_data )
...
def addHostKeyToKnownHostKeys( szPlatformName ):
    ...
    if sys.platform == 'win32':
        ...
        try:
            ...
            if g_bDebugVerbose:
                print("OpenKey %s succeeded" % mapRegKeyInfo[ 'regkey' ])
        except WindowsError:
            knownHostsKey = _winreg.CreateKey( _winreg.HKEY_CURRENT_USER,
                                               mapRegKeyInfo[ 'regkey' ] )
            if g_bDebugVerbose:
                print("CreateKey %s succeeded" % mapRegKeyInfo[ 'regkey' ])
        try:
            value, typ = _winreg.QueryValueEx( knownHostsKey, mapRegKeyInfo[ 'valueName' ] )
            # Validate the stored key matches expected (critical for security)
            if typ != _winreg.REG_SZ or value != mapRegKeyInfo[ 'value' ]:
                raise ValueError("SSH host key mismatch")
            if g_bDebugVerbose:
                print("QueryValueEx %s succeeded" % mapRegKeyInfo[ 'valueName' ])
            ...
            if g_bDebugVerbose:
                print("SetValueEx %s=%s succeeded" % ( mapRegKeyInfo[ 'valueName' ],
                                                       mapRegKeyInfo[ 'value' ] ) )
        try:
            _winreg.CloseKey( knownHostsKey )
            if g_bDebugVerbose:
                print("CloseKey succeeded")
        except WindowsError:
            pass
    else:
        # Posix: should add key to ~/.ssh/known_hosts
        raise NotImplementedError("addHostKeyToKnownKeys for posix not implemented")


def calculateRemoteView( szRemoteClientName ):
    global g_BranchSpec
...
def main():
    ...
    for o, a in opts:
        ...
        if o in ( "-e", "--exhaustive" ):
            g_lRemotePlatforms = list(g_mapPlatformToBuildHostInfo.keys())
        if o in ( "-p", "--platform" ):
    ...
    try:
        p4path = p4helpers.P4Where( os.path.join( szLocalClientRoot, r"src\..." )  )
    except Exception:
        try:
    ...
    if g_bVerbose:
        print("building configuration(s) %s on platform(s) %s" % ( g_lBuildConfigs, g_lRemotePlatforms ))

    szLocallySyncedRevision = p4helpers.GetSyncedRevision( szLocalClientRoot )

    szPickleFile = 'vcodepickle_%s.zip' % szLocalClientName
    if g_bVerbose:
        print("pickling local changes to %s..." % szPickleFile, end=' ')
    (ret, stdout_data, stderr_data) = runLocalCommand( '%s --backup --file %s -c %s --exclude "*.exe" --exclude "*.dll" --exclude "*.pdb"  --include "*/src/*"' % ( g_szVCodePickle, szPickleFile, szP4ChangeToSync ) )
    if g_bVerbose:
        sys.stdout.flush()
        print("done.")
    ...
    for platform in g_lRemotePlatforms:
        ...
        if g_bDebugVerbose:
            print("adding remote host key for %s to known keys..." % platform)
        ...
        if not os.path.exists( szSSHPrivKeyFile ):
            ...
            if g_bVerbose:
                print("creating ssh public/private keypair...")
            (ret, stdout_data, stderr_data) = runRemoteCommand( szUserName, szRemoteHost, g_szFetchSSHPrivateKeyScriptlet, bPromptForPasswd = True )
        
            if not g_bSimulateOnly:
                assert( len(stdout_data) )
                assert( not g_bBatchMode )
                with open( szSSHPrivKeyFile, "w", encoding='utf-8' ) as f:
                    f.write( stdout_data )
        ...
        if g_bDebugVerbose:
            print("adding private key to ssh authentication agent...")
        (ret, stdout_data, stderr_data) = runLocalCommand( "%s %s" % ( g_szSSHAgentAddKeyCmd, szSSHPrivKeyFile ), bWait = False )

        # figure out what the remote view should look like
        if g_bDebugVerbose:
            print("calculating remote view...")
        szRemoteView = calculateRemoteView( szRemoteClientName )
        if g_bDebugVerbose:
            print("remote view: %s" % szRemoteView)
        
        # do the magic substitution on the client create scriptlet
        if g_bVerbose:
            print("syncing remote p4 client spec...", end=' ')
        ...
        (ret, stdout_data, stderr_data) = runRemoteCommand( szUserName, szRemoteHost, szClientCreateScriptlet )
        if g_bVerbose:
            sys.stdout.flush()
            print("done.")
        ...
        if platform == "osx":
            if g_bVerbose:
                print("checking if vsign is set up...", end=' ')
            szRemoteCheckVsignScriptlet = g_szRemoteVsignTestTemplate % {	"P4CLIENT" : szRemoteClientName }
            (ret, stdout_data, stderr_data) = runRemoteCommand( szUserName, szRemoteHost, szRemoteCheckVsignScriptlet, False, False )
            if g_bVerbose:
                sys.stdout.flush()
                print("done.")
        
            if ret != 0:
                passphrase = input("Please enter code signature passphrase for public universe: ")
                if g_bVerbose:
                    print("setting vsign passphrase...", end=' ')
                szremoteSetVsignPassphraseScriptlet = g_szRemoteVsignSetPassphraseTemplate % {  "P4CLIENT" : szRemoteClientName,
                                                                                                "PASSPHRASE": passphrase}
                (ret, stdout_data, stderr_data) = runRemoteCommand( szUserName, szRemoteHost, szremoteSetVsignPassphraseScriptlet, False, False )
                if g_bVerbose:
                    sys.stdout.flush()
                    print("done.")
            
        if g_bVerbose:
            print("reverting any open files on remote p4client...", end=' ')
        szClientSyncScriptlet = g_szRemoteP4ScriptletTemplate % { "P4CLIENT" : szRemoteClientName,
                                                                  "P4COMMAND": 'revert ...' }
        (ret, stdout_data, stderr_data) = runRemoteCommand( szUserName, szRemoteHost, szClientSyncScriptlet )
        if g_bVerbose:
            sys.stdout.flush()
            print("done.")
        
        if g_bVerbose:
            print("syncing remote p4client to changelist %s (this could take a while)..." % szLocallySyncedRevision, end=' ')
        szClientSyncScriptlet = g_szRemoteP4ScriptletTemplate % { "P4CLIENT" : szRemoteClientName,
        ...
        if g_bVerbose:
            print("transferring pickled changes...", end=' ')
        (ret, stdout_data, stderr_data) = runLocalCommand( "%s %s %s@%s:P4Clients/%s/" % ( g_szSCPBin, szPickleFile, szUserName, szRemoteHost, szRemoteClientName ) )
        if g_bVerbose:
            sys.stdout.flush()
            print("done.")
        
        if g_bVerbose:
            print("applying changes to remote client...", end=' ')
        szUnpickleScriptlet = g_szRemoteUnpickleScriptletTemplate % { "P4CLIENT"  : szRemoteClientName,
                                                                      "PICKLEFILE": szPickleFile }
        (ret, stdout_data, stderr_data) = runRemoteCommand( szUserName, szRemoteHost, szUnpickleScriptlet )
        if g_bVerbose:
            sys.stdout.flush()
            print("done.")
        ...
        for buildConfig in g_lBuildConfigs:
            if g_bVerbose:
                print("running remote %s build..." % buildConfig, end=' ')
            ...
            (ret, stdout_data, stderr_data) = runRemoteCommand( szUserName, szRemoteHost, szBuildScriptlet, bCheckReturn = False )
            if g_bVerbose:
                sys.stdout.flush()
                print("done.")
            ...
            if stderr_data:
                ...
                errlines = stderr_data.split( "\n" )

                (fd, name) = tempfile.mkstemp( suffix=".txt" )
                # Write as bytes for Python 3 compatibility
                os.write( fd, (NL.join( errlines )).encode('utf-8') )
                os.close( fd )
                ...
                if ret != 0:
                    if not g_bBatchMode:
                        resp = input( "display full build log (Y/N)? " )
                        if resp.lower() == "y":
