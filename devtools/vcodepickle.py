# First off, I have no idea what all of my changes are
# Deprecated syntax replaced, logic errors, bla bla bla
#!/usr/bin/env python
...
def CreateChangelist(comment):
    ...
    po.stdin.close()
    theOutput = po.stdout.readline().decode('utf-8')
    ret = po.wait()
    if ret == 0:
...
def GetChangelistComment(nChangelist):
    ...
    lines = [line.decode('utf-8') for line in po.stdout.readlines()]
    ret = po.wait()
    if ret != 0:
...
def GetReadableChangelistComment(text):
    ...
    for i in range(len(text)):
        ...
        if validchars.find(testchar) == -1:
            ...
            outstr += testchar
        prevchar = outstr[-1:] if outstr else ''
...
def ShowUIForFilename():
    ...
    if g_UISaveDirKey not in g_Settings:
    ...
    if not os.path.exists(theDirName):
        theinput = input("Directory %s doesn't exist. Create? (Y/n) " % theDirName)
    ...
    #
    if os.path.exists(g_ZipFilename):
        theinput = input("%s already exists. Overwrite? (y/N)" % g_ZipFilename)
...
class VCodePickle:
    def __init__(self):
        self.theZipFile = None
        self.filesDeleted = {}
        self.filesAdded = {}
        self.filesEdited = {}
        self.changelistComment = ''
        self.relativeToDepotFileMap = {}
...
##
# Given a 'zf' instance, copy data from the 'name' to the
# 'out' stream.

def explode(out, zf, name):
    ...
    filepos = zf.fp.tell()
    zf.fp.seek(zinfo.header_offset, 0)
    ...
    fname = zf.fp.read(fheader[_FH_FILENAME_LENGTH])
    if fheader[_FH_EXTRA_FIELD_LENGTH]:
        zf.fp.read(fheader[_FH_EXTRA_FIELD_LENGTH])
    ...
    if decoder:
        out.write(decoder.decompress(b'Z'))
...
def WriteFileFromZip(theZipFile, srcFilename, sDestFilename):
    try:
        with open(sDestFilename, 'wb') as f:
...
def HandleNewTree(thePickle):
    ...
    if g_FromTree is None:
        return
        
    print("-changetree is translating these files:\n")
    
    for k in thePickle.relativeToDepotFileMap:
        ...
        if sFromFilename[:len(g_FromTree)].lower() == g_FromTree.lower():
            ...
            print("Used -changetree [%s] [%s], but file %s does not come from the source tree." % (g_FromTree, g_ToTree, sFromFilename))
            sys.exit(1)

    # Translate the revision number to a changelist number for edited files.
    newEdited = {}
    for k in thePickle.filesEdited:
...
def PromptForOpenZipFilename(sClientRoot, promptDir):
    if "wx" not in sys.modules:
        print("Couldn't load wx, no UI available.", file=sys.stderr)
        sys.exit(1)
...
def DoRestore(sClientRoot, sZipFilename):
    ...
    if openedForEdit and not g_bBinsOnly:
    ...
    if g_bVerbose:
        print("Adding %d files..." % len(thePickle.filesAdded))
    for k in thePickle.filesAdded:
        sDepotFilename = thePickle.relativeToDepotFileMap[k]
        ...
        if DoesFileExistInPerforce(sDepotFilename):
            ...
            if sLocalClientFilename is None:
    ...
    if g_bVerbose:
        print("Deleting %d files..." % len(thePickle.filesDeleted))
    for k in thePickle.filesDeleted:
        sDepotFilename = thePickle.relativeToDepotFileMap[k]
    ...
    if g_bVerbose:
        print("Editing %d files..." % len(thePickle.filesEdited))
    for k in thePickle.filesEdited:
        ...
        p4helpers.CheckPerforceReturn('p4 sync %s' % sDepotFilename)
        if g_bVerbose:
            print("\t%s#%s" % (k, revisionNumber))
        
    thePickle.theZipFile.close()

    PrintQueuedWarnings()
            
    if g_FromTree is not None:
        print("\n")
...
def CheckFileExcludedByPatterns(sFilename):
    ...
    if not bInclude:
        if g_bVerbose:
            print("skipping %s, doesn't match any include pattern" % sFilename)
        return True

    for excludePattern in g_ExcludeGlobPatterns:
        if glob.fnmatch.fnmatch(sFilename, excludePattern):
            if g_bVerbose:
                print("skipping %s matches exclude pattern %s" % (sFilename, excludePattern))
            return True
...
def DoDiff(sZipFilename, sDiffDir):
    ...
    if os.path.exists(sDiffDir):
        theinput = input("Warning: This will clear out %s. Proceed (y/N)? " % sDiffDir)
        if theinput == '' or theinput.lower() == 'n':
            print("Exiting.")
            sys.exit(0)
        
        # Delete all the files.
        subprocess.call('rd /s /q "%s"' % sDiffDir, shell=True)
                
    # Create the directories.
    os.makedirs(sTempFilesDir)
    
    # Dump out the changelist description.
    with open(os.path.join(sDiffDir, "changelist.txt"), 'wt', encoding='utf-8') as f:
        f.write(thePickle.changelistComment)
    
    # Dump out all the files.
    for sFilename in thePickle.filesEdited:
        ...
        with open(sBatchFilename, 'wt', encoding='utf-8') as fp:
            fp.write("""        
            @echo off
            setlocal
            """)
    ...
    subprocess.call('start %s' % sDiffDir, shell=True)
...
def main():
    ...
    if (g_RunMode == RUNMODE_RESTORE or g_RunMode == RUNMODE_EMPTY) and not g_ZipFilename and not bPrompt:
        print("\n*** file parameter is required in restore/empty mode ***\n\n\n", file=sys.stderr)
        ShowUsage()
        sys.exit(1)

    if g_Changelist and g_bBinsOnly and g_RunMode == RUNMODE_BACKUP:
        print("\n*** -c and --bins are mutually exclusive ***\n\n\n", file=sys.stderr)
        ShowUsage()
        sys.exit(1)

    if (g_IncludeGlobPatterns or g_ExcludeGlobPatterns) and g_RunMode != RUNMODE_BACKUP:
